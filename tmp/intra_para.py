...
class AlphaIntraExample(ExprScriptBase):
    def __init__(self, cfg):
        ...
        self.iter1 = dr.GetData("Intervalstats.skew_p")
        self.iter2 = dr.GetData("Intervalstats2.num_highP")
    ...
    def DailyRun(self, di):
        ...
        skew = lambda x: np.nanmean(np.power((x-np.nanmean(x, axis = 1 ))/Op.std(x),3), axis = 0)

        para1 = self.iter1[di][0:48][0:uv.instsz]
        para2 = self.iter2[di][0:48][0:uv.instsz]

        self.alpha = Op.mean(para1/para2)/Op.std(para1/para2)
