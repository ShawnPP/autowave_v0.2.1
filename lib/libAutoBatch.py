...
class AutoBatch:
    ...
    def __init__(self, config_path = None, pnl_prefix = "zx"):
    ...
    def generate_zxconfig(self, script_paths, pnl_name, startdate, enddate, adv=False, expression_negative=False, expression = None):
        ...
        if adv:
            ...
            if self.config.delay == "0":
                alphas[0].set("delay", "0")
                alphas[1].set("delay", "0")
                alphas[0].set("uId", "TOP2000Filterednoon")
                alphas[0].set("uId", "TOP2000")
        else:
            root.find('{pysim}Port/{pysim}Alpha').set('id', pnl_name)
            if self.config.delay == "0":
                root.find('{pysim}Port/{pysim}Alpha').set('delay', '0')
                root.find('{pysim}Port/{pysim}Alpha').set('uId', 'TOP2000Filterednoon')
    ...
    def alphatest(self, fitMode = False, *args):
        pair, expr, index, processed_index  = args
        self.alpha = Alpha()
        self.alpha.config = self.config
        self.alpha.pair = pair
        alphaInfo = {}
        alphaInfo["performancePct"] = None
        alphaInfo["performanceTrade"] = None
        alphaInfo["pnlCorr"] = None
        alphaInfo["pnlVa"] = None
        ...
        self.alpha.pnl_name = time.strftime(f'{self.pnl_prefix}%Y-%m-%d-%H%M%S', time.localtime())
        alphaInfo["pnlName"] = self.alpha.pnl_name
        ...
        remote_zxconfig_path, self.alpha.expression = self.generate_zxconfig(script_paths=remote_script_paths,
                                                                             ...
                                                                             startdate='20180101', enddate='TODAY',
                                                                             expression=expr)
        ...
        if sharpe < self.config.bottom_sharpe * -1:
        ...
        if sharpe > self.config.bottom_sharpe:
            ...
            if self.alpha.pnl_flag:
                ...
                if self.alpha.corr_flag:
                    ...
                    alphaInfo["performance_pct"] = self.alpha.performance_pct
                    alphaInfo["performance_trade"] = self.alpha.performance_trade
                    if self.alpha.pcttrade_flag:
                        if fitMode == False:
                            print('>>>>>> Netting Return Test >>>>>>', flush=True)
                            SSH.execute(f'mkdir /dropbox/xiongzhang/addvalue_wave/{self.alpha.pnl_name}')
                            SSH.execute(f'cp {self.config.remote_dir}/dumppath/{self.alpha.pnl_name}.N,5120f /dropbox/xiongzhang/addvalue_wave/{self.alpha.pnl_name}')
                            '''write the key output to file'''
        alphaInfo["pnlFlag"] = self.alpha.pnl_flag
        alphaInfo["corrFlag"] = self.alpha.corr_flag
        alphaInfo["pctTradeFlag"] = self.alpha.pcttrade_flag
        alphaInfo["performance"] = self.alpha.performance
        alphaInfo["intraparas"] = self.config.intraparas
        alphaInfo["tempaltePyPath"] = self.config.script_template_path
        alphaInfo["expression"] = self.alpha.expression
        '''
        end of a pair"
    ...
    def run(self,args,fitMode = False):
        '''
        ...
        self.progress_output.write(self.output_corr_filename + '\n')
        self.progress_output.flush()
        print('progress save to', self.progress_output.name, flush=True)
        pathNetting = self.config.local_dir + self.output_filename
        pathCorr = self.config.local_dir + self.output_corr_filename

        for expr in self.config.expressions:
            for index, pair in enumerate(self.dataname_pairs):
                args = (pair, expr, index, processed_index)
                message = self.alphatest(fitMode, *args)
                if message != "pass the index":
                    if message["corrFlag"] == True:
                        Alpha.write(message, outputFilePath=pathCorr)
                    if message["pctTradeFlag"] == True:
                        Alpha.write(message,  outputFilePath=pathNetting)
